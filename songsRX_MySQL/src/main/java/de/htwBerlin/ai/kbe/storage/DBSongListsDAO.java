...
public class DBSongListsDAO implements InterfaceSongListsDAO {
	...
	@Override
	public SongLists findSongListById(String userId, Integer songListId, boolean securityType) {
		EntityManager entMan = entManFact.createEntityManager();
		try {

			TypedQuery<SongLists> query = entMan.createQuery(
					"SELECT c FROM SongLists c where user_id = " + "'" + userId + "' and id=" + songListId,
					SongLists.class);
			if (securityType) {
				query = entMan.createQuery("SELECT  c FROM SongLists c where user_id = " + "'" + userId
